...
$no=1;
if($loker == 'NULL'){
$query=mysqli_query($con,"SELECT a.idDaftar, b.nisn, b.nama, d.nmJurusan, c.perusahaan, c.nmLoker, a.status FROM pendaftaran_loker a, siswa b, lowongan c, jurusan d WHERE a.idAnggota=b.idSiswa AND a.idLoker=c.idLowongan AND b.jurusan=d.idJurusan AND a.status='4' ORDER BY c.idLowongan ASC");
}else{
  
$query=mysqli_query($con,"SELECT a.idDaftar, b.nisn, b.nama, d.nmJurusan, c.perusahaan, c.nmLoker, a.status FROM pendaftaran_loker a, siswa b, lowongan c, jurusan d WHERE a.idAnggota=b.idSiswa AND a.idLoker=c.idLowongan AND b.jurusan=d.idJurusan AND a.status='4' AND c.idLowongan = '$loker' ORDER BY c.idLowongan ASC");
}
