...
$pdf->Cell(4.5, 0.8, 'Status Perusahaan', 1, 0, 'C');
$pdf->Cell(4.5, 0.8, 'Telepon', 1, 0, 'C');
$pdf->Cell(5, 0.8, 'Tgl. Kerjasama', 1, 1, 'C');
$pdf->SetFont('Arial','',10);
$no=1;
if($tahun == 'NULL'){
$query=mysqli_query($con,"SELECT * FROM perusahaan ORDER BY idPerusahaan DESC");
}else{
  
$query=mysqli_query($con,"SELECT * FROM perusahaan WHERE YEAR(tglKerjasama) = '$tahun' ORDER BY idPerusahaan DESC");
}
while($lihat=mysqli_fetch_array($query)){
$pdf->Cell(1, 0.8, $no, 1, 0, 'L');
$pdf->Cell(6, 0.8, $lihat['nmPerusahaan'], 1, 0,'L');
$pdf->Cell(5, 0.8, $lihat['email'], 1, 0,'L');
$pdf->Cell(4.5, 0.8, $lihat['stsPerusahaan'],1, 0, 'L');
